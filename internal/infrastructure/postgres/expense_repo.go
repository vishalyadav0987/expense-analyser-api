...
func (r *ExpenseRepository) GetCategoryById(ctx context.Context, categoryId string, userId string) (*domain.Category, error) {
	query := `
		SELECT id, user_id, name, type, created_at FROM categories WHERE user_id = $1 AND categoryId = $2
	`
...
func (r *ExpenseRepository) CreateExpense(ctx context.Context, exp *expense.Expense) (string, error) {
	query := `
		INSERT INTO expenses (id, user_id, amount, category_id, description, payment_mode, date)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
	`
	result, err := r.db.ExecContext(ctx, query, exp.ID, exp.UserID, exp.Amount, exp.CategoryID, exp.Description, exp.PaymentMode, exp.Date)
	if err != nil {
		return "", fmt.Errorf("failed to insert expense: %w", err)
	}

	transactionId, err := result.LastInsertId()
	str := fmt.Sprintf("%d", transactionId)
	return str, nil
}
